...
public class Joypurhat extends AppCompatActivity {
  ...
  /*  @Override
    ...
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate ( savedInstanceState );
        setContentView ( R.layout.activity_joypurhat );
        database = FirebaseDatabase.getInstance().getReference().child("projectjonobak").child("posts").child("joypurhat");
    ...
    protected void onStart() {
        super.onStart();
        FirebaseRecyclerAdapter<Post, Joypurhat.PostViewHolder> firebaseRecyclerAdapter = new FirebaseRecyclerAdapter<Post, Joypurhat.PostViewHolder>(
                Post.class, R.layout.post_row, Joypurhat.PostViewHolder.class, database
        ) {
            @Override
            protected void populateViewHolder(Joypurhat.PostViewHolder viewHolder, Post model, int position) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if(item.getItemId() == R.id.action_add) {
            Intent intent = new Intent(Joypurhat.this, PostActivity.class);
            intent.putExtra("PARENT_ACTIVITY", "joypurhat");
