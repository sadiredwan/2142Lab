...
public class Pabna extends AppCompatActivity {
   ...
   /* @Override
    ...
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate ( savedInstanceState );
        setContentView ( R.layout.activity_pabna );
        database = FirebaseDatabase.getInstance().getReference().child("projectjonobak").child("posts").child("pabna");
    ...
    protected void onStart() {
        super.onStart();
        FirebaseRecyclerAdapter<Post, Pabna.PostViewHolder> firebaseRecyclerAdapter = new FirebaseRecyclerAdapter<Post, Pabna.PostViewHolder>(
                Post.class, R.layout.post_row, Pabna.PostViewHolder.class, database
        ) {
            @Override
            protected void populateViewHolder(Pabna.PostViewHolder viewHolder, Post model, int position) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if(item.getItemId() == R.id.action_add) {
            Intent intent = new Intent(Pabna.this, PostActivity.class);
            intent.putExtra("PARENT_ACTIVITY", "pabna");
