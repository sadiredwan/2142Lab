...
public class Sirajgang extends AppCompatActivity {
   ...
   /* @Override
    ...
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate ( savedInstanceState );
        setContentView ( R.layout.activity_sirajgang );
        database = FirebaseDatabase.getInstance().getReference().child("projectjonobak").child("posts").child("sirajgang");
    ...
    protected void onStart() {
        super.onStart();
        FirebaseRecyclerAdapter<Post, Sirajgang.PostViewHolder> firebaseRecyclerAdapter = new FirebaseRecyclerAdapter<Post, Sirajgang.PostViewHolder>(
                Post.class, R.layout.post_row, Sirajgang.PostViewHolder.class, database
        ) {
            @Override
            protected void populateViewHolder(Sirajgang.PostViewHolder viewHolder, Post model, int position) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if(item.getItemId() == R.id.action_add) {
            Intent intent = new Intent(Sirajgang .this, PostActivity.class);
            intent.putExtra("PARENT_ACTIVITY", "sirajgang");
